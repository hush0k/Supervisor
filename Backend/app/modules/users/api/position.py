...
from Backend.app.modules.users.services.position import PositionServices
from Backend.app.modules.users.shcemas.position import PositionResponse, PositionCreate

router = APIRouter(prefix="/position", tags=["Position"])

def get_position_service(db: Annotated[AsyncSession, Depends(get_db)]) -> PositionServices:
    return PositionServices(db)

ServiceDep = Annotated[PositionServices, Depends(get_position_service)]

@router.post("/create", response_model=PositionResponse, status_code=http_status.HTTP_201_CREATED)
